...
func (m *metadata) DeleteCategoryWithTx(ctx context.Context, tx gdb.TX, id int64) (err error) {
	_, err = dao.Category.Ctx(ctx).TX(tx).Where(dao.Category.Columns().ID, id).Delete()
