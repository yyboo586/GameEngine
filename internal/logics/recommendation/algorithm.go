...
func (ra *RecommendationAlgorithm) GetPersonalizedRecommendations(ctx context.Context, userID int64, pageReq *model.PageReq) (outs []*model.Game, pageRes *model.PageRes, err error) {
	if pageReq.Page == 0 {
		pageReq.Page = 1
	}
	if pageReq.Size == 0 {
	...
	// 获取用户偏好
	userPreferences, err := ra.personalizationEngine.GetUserPreferences(ctx, userID)
	if err != nil {
		// 如果获取失败，返回热门推荐
	...
	// 基于用户偏好生成推荐
	outs, err = ra.personalizationEngine.GeneratePersonalizedRecommendations(ctx, userPreferences, pageReq.Size)
	if err != nil {
		return
	}
